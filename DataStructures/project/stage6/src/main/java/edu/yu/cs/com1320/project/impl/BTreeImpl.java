...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    public Value get(Key k) {
        ...
        if (entry != null) {
            if (entry.val == null && k instanceof URI) {
                try {
                    Value document = this.manager.deserialize(k);
                    if (document != null) {
                        this.put(k, document);
                    }
                    return document;
            ...
            /*
            if (entry.val instanceof URI && ((URI) entry.val).getScheme().equals("file")){
                try {
                    Value document = this.manager.deserialize(k);
                    doc.
                } catch (IOException e) {
        ...
        // internal node
            ...
            for (j = 0; j < currentNode.entryCount; j++) {
                ...
                if ((j + 1 == currentNode.entryCount) || less(key, currentNode.entries[j + 1].key)) {
                    ...
                    //will be inserted in the next slotNode newNode = this.put(currentNode.entries[j++].child, key, val, height - 1);
